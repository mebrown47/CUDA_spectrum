#!/usr/bin/env python3
"""
rfi_plot.py — Plot waterfall + PSD from cuda_spectrum output
Supports both CSV (--db single frame) and binary (-o binary multi-frame) input.

Examples:
    # Single frame from CSV (16M-point):
    python3 spectrum_plot.py rf_16M.csv --fft 16777216 --rate 10e6 --center 435e6

    # Multi-frame waterfall from binary (4M-point, 141 frames):
    python3 spectrum_plot.py example_4M_allframes.bin --binary --fft 4194304 --rate 10e6 --center 435e6

    # Zoom into a band:
    python3 spectrum_plot.py example_16M.csv --fft 16777216 --rate 10e6 --center 435e6 --zoom 434.5 435.5
"""
...
def plot_rfi(frames, fft_size, sample_rate, center_freq,
    ...
    if ax_wf is not None:
        ...
        # make_axes_locatable keeps colorbar from stealing plot width,
        # ensuring PSD and waterfall x-axes stay perfectly aligned
        divider_wf = make_axes_locatable(ax_wf)
        cax = divider_wf.append_axes('right', size='1.5%', pad=0.05)
        ...
        # Phantom axes on PSD panel — same size, invisible — matches x-axis width
        divider_psd = make_axes_locatable(ax_psd)
        cax_phantom = divider_psd.append_axes('right', size='1.5%', pad=0.05)
        cax_phantom.set_visible(False)
    ...
    out = save_path or 'rfi_analysis.png'
...
def main():
    ...
    ap.add_argument('--title', default='RFI Analysis 430-440 MHz')
    ap.add_argument('--save',  default='rfi_analysis.png',   metavar='FILE')
    ...
    print(f"rfi_plot.py  fft={args.fft:,}  rate={args.rate/1e6:.1f} Msps"
